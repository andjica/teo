...
import { Block, BlockTitle, Button, Card, CardContent, Icon, Page } from "framework7-react";
...
const [selectedVariants, setSelectedVariants] = useState({});
  ...
  const fetchNewProducts = async () => {
    try {
      const response = await get("second-hand/products"); // 👈 koristi get iz api.js
  ...
  return (
    <Page className="hide-scrollbar" name="secondHandProducts">
      <Navigate />
      <Category categories={categories} />
      <BlockTitle>Second Product Grid</BlockTitle>
      <Block className="grid grid-cols-2 grid-gap">
        {secondProducts.map((p) => (
